...
RUN pi-log (INPUT 'ê uma imagem DWF', INPUT TRUE).
    ...
    RUN pi-log(INPUT 'Caminho do arquivo AcmeCADConverter.exe: ' + IF FILE-INFO:FULL-PATHNAME <> ? THEN FILE-INFO:FULL-PATHNAME ELSE '?', INPUT TRUE).
                         ...
                         ASSIGN cCmd = FILE-INFO:FULL-PATHNAME
                             ...
                             + " /i" /* When you set this parameter and when you open the file and unable to find the Àshx fileÃ, it will no longer prompt the user, instead the name of such file will be written to a report file (if a report file needs to be so generated). */
...
PROCEDURE pi-gera-cabecalho-inspecao:
   ...
   run pi-cria-dados (fill("Ó", 119),           0, 1, "", 0, 0, 0).
END PROCEDURE.
...
procedure pi-gera-dados-inspecao:
   ...
   for each oper-ord where oper-ord.nr-ord-produ = ord-prod.nr-ord-produ no-lock
       ...
       run pi-soma-linha (1).
       run pi-cria-dados (fill("Ó", 119), 119, 1, "", 0, 0, 0).
       ...
       if can-find(first tt-exames) then do:
          ...
          run pi-cria-dados (fill("Ó", 119), 119, 0, "", 0, 0, 0).
       ...
       if last-of (oper-ord.nr-ord-produ) then do:
         ...
         if can-find(first tt-exames) then do:
            ...
            run pi-cria-dados (fill("Ó", 119), 119, 0, "", 8, 0, 0).

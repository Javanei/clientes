...
namespace VaultTools.vault.util
{
    class DownloadFast
    {
        ...
        public void DownloadFile(ADSK.File file, string fileDir)
        {
            NeodentUtil.util.LOG.debug(" ==== DownloadFile: fileDir=" + fileDir + ", file=" + file.ToString());
            VDF.Vault.Currency.Entities.FileIteration fileIter = new VDF.Vault.Currency.Entities.FileIteration(this.m_conn, file);
            VDF.Vault.Settings.AcquireFilesSettings settings = new VDF.Vault.Settings.AcquireFilesSettings(m_conn)
            {
                LocalPath = new VDF.Currency.FolderPathAbsolute(fileDir)
            };
